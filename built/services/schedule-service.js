...
var _nodeScheduleTz = _interopRequireWildcard(require("node-schedule-tz"));
...
class ScheduleService {constructor() {this.
  ...
  runSchedule(groupId, name, ruleString, text) {const _this$getRuleFromStri =
    ...
    (0, _nodeScheduleTz.scheduleJob)(name, rule, 'Asia/Shanghai', this.sendText.bind(null, groupId, text));
    return { hours, days };
  }
  ...
  cancelSchedule(name) {
    const job = _nodeScheduleTz.default.scheduledJobs[name];
    if (job) {
      job.cancel();
    }
  }

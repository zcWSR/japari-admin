...
AkhrUpdate = (_dec = (0, _plugin.Command)({ name: '更新明日方舟公招干员数据', command: 'akhrUpdate', type: 'all', info: '更新明日方舟公招干员数据', level: 3 }), _dec(_class = class AkhrUpdate {
  ...
  run(params, body, type) {var _this = this;return _asyncToGenerator(function* () {
      try {
        yield _akhrService.default.updateAkhrList();
        _this.sendMsg(body, type, '公招数据已更新');
      } catch (e) {
        _this.sendMsg(body, type, '公招数据更新失败');
